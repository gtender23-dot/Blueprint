...
export const MANIFEST = [
  ...
  { name: 'letter_logo_ui_smoke.mjs', tier: 'full', kind: 'pw', timeoutSec: 300, envKnown: true, note: 'TEST-STALE everywhere, NOT cloud (FULLGATE_TRIAGE 2026-08-17 item 7): enters through the RETIRED coach main-menu door (#btn-mm-newcoach — tree-only menu since W9 §12); PW auto-waits 30s then crash-tails. Fix is an M rewrite onto the tree path (#btn-mm-newtree), owed. envKnown kept only so the cloud runner reports it separately; a local red is EXPECTED until the rewrite lands.' },
  { name: 'saved_team_library_ui_smoke.mjs', tier: 'full', kind: 'pw', timeoutSec: 300, envKnown: true, note: 'TEST-STALE everywhere, NOT cloud (FULLGATE_TRIAGE 2026-08-17 item 8): clicks [data-mm-coach], which the tree-only menu never renders; the coach-home saved-team library screen it tests is unreachable UI. Fix is an M retarget (or retirement with owner sign-off), owed. Local red EXPECTED until then.' },
  { name: 'instant_classic_ui_smoke.mjs', tier: 'full', kind: 'pw', timeoutSec: 300, envKnown: true, note: 'TEST-STALE everywhere, NOT cloud (FULLGATE_TRIAGE 2026-08-17 item 9): clicks [data-mm-coach="…"], never rendered since W9 §12; the tree home has its own classics UI ([data-mm-tree-classic]) and the watch-viewer ids the probe uses afterward all survive. Fix is an M re-seed onto a tree, owed. Local red EXPECTED until then.' },
  ...
  { name: 'calendar_display_probe.mjs', tier: 'full', kind: 'pw', timeoutSec: 300, envKnown: true, note: 'TEST-STALE everywhere, NOT cloud (FULLGATE_TRIAGE 2026-08-17 item 10): its wizard script starts at the retired #btn-mm-newcoach and stalls before it ever reaches the D7-updated kickoff line ("watch"). Fix is S/M — replace the wizard\'s first steps with the tree entry — owed. Local red EXPECTED until then.' },
