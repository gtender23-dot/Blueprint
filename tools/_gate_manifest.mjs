...
export const MANIFEST = [
  ...
  { name: 'pass7_band_ab.mjs', tier: 'core', kind: 'node', args: ['300'], timeoutSec: 600,
    note: 'CURRENT pass A/B — swap this entry when the next pass ships its own' },
  ...
  { name: 'play_fidelity_probe.mjs', tier: 'core', kind: 'node', args: ['4'], timeoutSec: 300, note: 'THE CALL IS THE PLAY (owner mandate): gate=runnable, 1156-snap zero-miss fidelity, def deployment, gadget signatures, re-prompt pin. Killed the QB-audibles-out-of-coach-calls bug.' },
  ...
  { name: 'tipdrill_probe.mjs', tier: 'core', kind: 'node', args: ['6'], timeoutSec: 400,
    note: 'M25 tip-drill INT chain — pins Math.random, deterministic by construction. Amp/kill/live arms + box-score booking + viewer tip-then-pick staging.' },
  { name: 'tipdrill_ab.mjs', tier: 'core', kind: 'node', args: ['120'], timeoutSec: 900, seedFlaky: true,
    note: 'M25 feature A/B (NOT a band A/B — pass7_band_ab keeps the swap slot): INT drift up-and-small, PBU bounded by INTs gained, pts/comp%/plays flat. UNSEEDED (a pinned LCG phase-shifts the arms into different games) — hence seedFlaky.' },
  ...
  { name: 'playnow_smoke.mjs', tier: 'core', kind: 'pw', timeoutSec: 300 },
  ...
  { name: 'formation_playbook_probe.mjs', tier: 'full', kind: 'node', timeoutSec: 400 },
  ...
  { name: 'pass6_band_ab.mjs', tier: 'full', kind: 'node', timeoutSec: 600 },

  // ── full: world / season / saves / career ──────────────────────────────────
  { name: 'worldgen_check.mjs', tier: 'full', kind: 'node', timeoutSec: 300 },
  { name: 'thin_roster_check.mjs', tier: 'full', kind: 'node', timeoutSec: 300 },
  ...
  { name: 'commit_rate_test.mjs', tier: 'full', kind: 'node', timeoutSec: 300 },
