...
export const MANIFEST = [
  ...
  { name: 'defcard_fidelity_probe.mjs', tier: 'core', kind: 'node', args: ['40'], timeoutSec: 120,
    note: 'DOES THE CARD TELL THE TRUTH? (2026-08-21) Renders a defensive call card, plays that exact call through the M1 bench, and asserts the drawing and the snap agree — the question CREATOR_FIDELITY asked of the offense and nobody had ever asked of the defensive card. Four pins per subject: (A) the arrows the card draws equal the sim\'s modal rushN; (B1) every LINEMAN drawn rushing rushes on 60%+ of snaps, (B2) every EXTRA rusher drawn is blitz-eligible from that front — exactness is impossible without a roster, the sim picks among the eligible by pass-rush grade, so a live sheet passes resolved men through opts.blitzers and a book card falls back to DEF_BLITZ_ELIGIBLE order; (C) the sim plays the coverage the card NAMES on 90%+ of snaps and (C2) the card\'s man/zone drawing matches what that coverage is; (D) a card that draws a drop drops, one that does not, does not. It found four real defects on its first honest run: the fire-zone drop was an exchange on the backer path and a subtraction on the lineman path, the bring labels named absolute counts three fronts never rush, the card drew no fire-zone drop at all, and a man could be drawn blitzing and covering at once. Reads the coverage from the SNAP, never from covSlots\' `t` — "press" there is a technique a zone corner also carries. If it goes red on a card change, the drawing moved away from the game; if on an engine change, the game moved away from the drawing. Deterministic (pinned bench seeds).' },
  ...
  { name: 'coverage_monotonicity_check.mjs', tier: 'full', kind: 'node', timeoutSec: 240,
    note: 'Sweeps each coverage attribute 20->99 through the real routeDuel and asserts every one HELPS THE DEFENSE. 2026-08-21: it never called process.exit, so it always exited 0 and the gate always read OK — while printing "HELPS THE RECEIVER (INVERTED)" on the deep-zone AWR sweep every run since it was written. A probe that reports a defect and cannot go red is a log line, not a gate. The inversion is now fixed in sepgeo.js (the 2026-08-14 mean-neutral term was the right idea at a fifth of the size it needed — 0.0019 bent the curve 0.15 against a raw inversion of 0.53; 0.0103 is derived to give deep zone the same AWR slope medium zone already has, -0.00362 vs -0.00365), so wiring the exit code adds no red today. Deep stays bumpy in the middle (AWR 80 above AWR 60) because the underlying arming is positional — fixing that is the real repair and this check is what will prove it when someone does.' },
  ...
  { name: 'timeout_screen_smoke.mjs', tier: 'core', kind: 'pw', timeoutSec: 300, note: 'THE LIVE TIMEOUT SURFACE. 2026-08-17 fixed the unwired #to-adjust-root chips (FULLGATE_TRIAGE item 1). 2026-08-21 (owner session) found the three defects UNDER that fix and this smoke now pins all of them: (A) the ⏱️ chip existed only on the two call sheets, so a coach who forgot it when he sent the play in had no way to reach it — it is now in the LIVE watch bar for the whole call stage and fires by itself at the whistle; (B) renderTimeoutAdjustOverlay was gated on state.ui.showCallSheet, which the dominant live path NEVER sets (state.js writes state.ui.liveWatch instead), so the adjustments screen did not render on a live coached game at all — it only appeared when an earlier non-live stop had left the flag true, which is exactly why it read as intermittent; (C) "Rest of Game" rendered the token\'s game-local plan but WROTE the school\'s season plan through setPlanFields — the chip never lit, the change never reached the game in progress, and it silently persisted into next week. All three game-scoped roots (kickoff, halftime, timeout) now pass wireDefaultsListeners({inPlace:true}). Promoted full->core: this is the play-caller\'s clock, and three separate defects hid here behind one fix. The tempo check is plan-INDEPENDENT by construction (it flips whichever chip is currently OFF) — the old hard-coded "Hurry" version failed on any walk whose plan already ran Hurry, an unseeded probe bug that cost a real debugging cycle. envKnown DROPPED 2026-08-21: it walks and passes cleanly in the cloud sandbox, three runs for three.' },
