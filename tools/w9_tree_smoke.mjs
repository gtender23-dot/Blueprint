...
try {
  ...
  // ── 1. The menu — the collapsed start flow ──────────────────────────────
  // The tree is the ONLY start path now: no legacy coach list, no "name the
  // tree" step. Start a dynasty, enter your name, and the LAST NAME becomes the
  // tree while the wizard opens on a D3 job (growth rule 1).
  check(await page.locator('#btn-mm-newtree').count() === 1, 'the main menu leads with START A DYNASTY');
  check(await page.locator('[data-mm-coach]').count() === 0, 'the legacy YOUR COACHES list is hidden — the tree is the only start');
  await page.click('#btn-mm-newtree'); await page.waitForTimeout(200);
  check(await page.locator('#mm-nt-first').count() === 1 && await page.locator('#mm-nt-last').count() === 1,
    'starting asks for the coach name, not a tree name');
  await page.fill('#mm-nt-first', 'Sam');
  await page.fill('#mm-nt-last', 'Grove');
  await page.click('#mm-nt-create'); await page.waitForTimeout(400);
  // Founding creates the tree (named for the last name) + the trunk coach and
  // goes straight to the wizard. Verify the store, not the screen.
  const founded = await page.evaluate(async () => {
    const CP = await import('./js/engine/coachprofile.js');
    const { state } = await import('./js/state.js');
    const t = CP.listTrees()[0];
    const c = state._coachId ? CP.getCoach(state._coachId) : null;
    return { treeName: t?.name || null, treeId: t?.id || null, stateTreeId: state._treeId || null,
             coachLast: c?.name?.last || null, onWizard: (state.ui?.view === 'newgame') };
  });
  check(founded.treeName === 'Grove', `the tree is named for the coach's last name (${founded.treeName})`);
  check(founded.stateTreeId === founded.treeId && founded.coachLast === 'Grove',
    'the trunk coach is minted and the run is bound to the tree, in one step');
  ...
  // The Coach's Office is now two tabs (Identity | Program); the tree panel
  // lives on the PROGRAM tab. Switch to it before checking.
  await page.evaluate(async () => {
    const s = await import('./js/state.js');
    if (s.setGroupTab) s.setGroupTab('program', 'program');
    s.rerender && s.rerender();
  });
  await page.waitForTimeout(300);
  const office = await page.locator('.tree-card').innerText().catch(() => '');
  check(!!office, 'the coach\'s office (Program tab) renders the tree panel');
