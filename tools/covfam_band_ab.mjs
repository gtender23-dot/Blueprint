// covfam_band_ab.mjs — PASS 3 band gate: AI-vs-AI league stats with coverage
// families LIVE vs KILLED (__noCovFamilies). Band rule (roadmap): the moment
// AI gains a new scheme layer — here the family-flavored coverage call and
// the Victory/Prevent call — prove the stat bands didn't move. The switch is
// data-layer clean: a killed world's AI never authors the ingredients at all
// (buildAISignatureCalls gates generation), so this compares generated-world
// vs generated-world, the defcall_band_ab precedent exactly.
// Run: node tools/covfam_band_ab.mjs [N-games-per-arm]   (default 300)
...
function arm(kill) {
  ...
  const acc = { g: 0, pts: 0, rush: 0, pass: 0, ints: 0, plays: 0, to: 0, comp: 0, att: 0 };
  try {
    for (let i = 0; i < N; i++) {
      ...
      for (const [st, score] of [[res.homeStats, res.homeScore], [res.awayStats, res.awayScore]]) {
        ...
        acc.comp += st.compAtt ?? 0; acc.att += st.passAtt ?? 0;
      }
    }
  } finally { delete globalThis.__noCovFamilies; }
  return acc;
}
const line = (l, a) => console.log(
  `${l.padEnd(24)} pts ${(a.pts / a.g).toFixed(1)}  rush ${(a.rush / a.g).toFixed(1)}  pass ${(a.pass / a.g).toFixed(1)}` +
  `  comp% ${(100 * a.comp / (a.att || 1)).toFixed(1)}  plays ${(a.plays / a.g).toFixed(1)}  INT ${(a.ints / a.g).toFixed(2)}  TO ${(a.to / a.g).toFixed(2)}`);
console.log(`AI vs AI, ${N} games per arm (per-team-game averages):`);
const live = arm(false);
line('coverage families LIVE', live);
const dead = arm(true);
line('__noCovFamilies KILLED', dead);
const drift = (get) => Math.abs(get(live) - get(dead));
const pts = drift((a) => a.pts / a.g), rush = drift((a) => a.rush / a.g), pass = drift((a) => a.pass / a.g), comp = drift((a) => 100 * a.comp / (a.att || 1));
console.log(`\ndrift: pts ${pts.toFixed(2)}  rush ${rush.toFixed(2)}  pass ${pass.toFixed(2)}  comp% ${comp.toFixed(2)}`);
const ok = pts < 2.0 && rush < 8 && pass < 10 && comp < 1.5;
