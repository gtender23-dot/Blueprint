// defcall_band_ab.mjs — PASS 2 band gate: AI-vs-AI league stats with
// signature calls LIVE vs KILLED (__noDefCalls). The roadmap band rule:
// the moment AI gains a new scheme layer, prove the stat bands didn't move.
// Run: node tools/defcall_band_ab.mjs [N-games-per-arm]   (default 300)
...
function arm(kill) {
  ...
  const acc = { g: 0, pts: 0, rush: 0, pass: 0, sacks: 0, ints: 0, plays: 0, to: 0 };
  try {
    for (let i = 0; i < N; i++) {
  ...
  } finally { delete globalThis.__noDefCalls; }
  return acc;
}
const line = (l, a) => console.log(
  `${l.padEnd(22)} pts ${(a.pts / a.g).toFixed(1)}  rush ${(a.rush / a.g).toFixed(1)}  pass ${(a.pass / a.g).toFixed(1)}` +
  `  plays ${(a.plays / a.g).toFixed(1)}  INT ${(a.ints / a.g).toFixed(2)}  TO ${(a.to / a.g).toFixed(2)}`);
console.log(`AI vs AI, ${N} games per arm (per-team-game averages):`);
const live = arm(false);
line('signature calls LIVE', live);
const dead = arm(true);
line('__noDefCalls KILLED', dead);
const drift = (k, get) => Math.abs(get(live) - get(dead));
const pts = drift('pts', (a) => a.pts / a.g), rush = drift('rush', (a) => a.rush / a.g), pass = drift('pass', (a) => a.pass / a.g);
console.log(`\ndrift: pts ${pts.toFixed(2)}  rush ${rush.toFixed(2)}  pass ${pass.toFixed(2)}`);
const ok = pts < 2.0 && rush < 8 && pass < 10;
