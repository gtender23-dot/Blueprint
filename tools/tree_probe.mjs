...
hdr('T5 — retirement is the harvest, and it happens exactly once');
{
  ...
  // Give the trunk a real career to bank. (W9 fold: `culture` merged into
  // `motivator`, so the career lives on the live motivator axis. 900 = the old
  // 600 culture + 300 motivator, tested on the axis that actually exists.)
  CP.addDnaXP(prof.id, { motivator: 900, groundPound: 150 });
  ...
  const bankM = Math.round(900 * C.TREE.BANK_SHARE), bankG = Math.round(150 * C.TREE.BANK_SHARE);
  check(st.tree.dna.axes.motivator === bankM && st.tree.dna.axes.groundPound === bankG,
    `the career banks into the tree at BANK_SHARE ${C.TREE.BANK_SHARE} (motivator ${st.tree.dna.axes.motivator} of 900, groundPound ${st.tree.dna.axes.groundPound} of 150)`);
  ...
  CP.bankIntoTree(st.tree, prof.id, { seasons: 4, share: C.TREE.BANK_SHARE });
  check(st.tree.dna.axes.motivator === bankM && st.tree.ledger.length === 1,
  ...
  const inherit = CP.dnaInheritance(st.tree, { seasonsUnderTree: 0 });
  const expected = Math.round(bankM * C.TREE.INHERIT_SHARE);
  check(inherit.axes.motivator === expected,
    `the next coach on this tree starts with a SHARE of it (motivator ${inherit.axes.motivator} of ${st.tree.dna.axes.motivator}) — the protégé effect`);
  ...
  // The star cap (DNA TREE §4): a protégé opens at up to ★★, never ★★★/💎 —
  // those he earns himself. The reason a fourth-generation tree is not
  // strictly easier.
  st.tree.dna.axes.motivator = 99999;
  const capped = CP.dnaInheritance(st.tree, { seasonsUnderTree: 30 });
  check(CP.dnaStarTier(capped.axes.motivator) <= C.TREE.INHERIT_CAP_STAR,
    `no protégé inherits past ${CP.dnaStarLabel(C.TREE.INHERIT_CAP_STAR)} however rich the tree gets (would-be ${CP.dnaStarLabel(CP.dnaStarTier(Math.round(99999 * C.TREE.INHERIT_MAX)))}) — a head start, never a career`);
}
