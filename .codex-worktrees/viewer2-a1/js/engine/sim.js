...
import { FRONT_PRESSURE_SIGNATURE, defUnitStrengthSchemeFit, getDefWeights, getMatchupEdge, getOffWeights, getSituationalMod, offPersonnelClass, offUnitStrengthRoles, pickedVariation, resolveDefPersonnel, resolvePersonnel, rollFormation, selectDefFront, variationPassLeanDelta } from './formations.js';
...
function simulateDrive(offense, defense, gameState, log, opts = {}) {
  ...
  while (gameState.clock > 0 || down === 4 && !fourthDecided && gameState.half >= 2) {
    ...
    // byte-identical to today. AI never sets it, so the standing band gate only
    // moves when a variation-bearing gameplan is actually in play.
    const offVar = (forcedCall == null ? void 0 : forcedCall.variation) || pickedVariation(offEff.offFormations, offFormationId);
