...
import { assembleWorldSources, generateWorld } from '../../engine/world.js';
...
function rerollSchool(t, division) {
  ...
  const pick = src.schools[Math.floor(Math.random() * src.schools.length)];
  if (pick) { t.name = pick.name; t.nick = pick.nick; t.colors = pick.colors; t.state = pick.state; t.crestSeed = Math.random().toString(36).slice(2, 8); }
}
...
function blueprintFromEditor(d) {
  return { schemaVersion: 1, name: d.name, division: d.division,
    ...
    teams: d.teams.map((t) => ({ id: t.id, name: t.name, division: d.division, conf: t.conf, prestige: t.prestige, nick: t.nick, colors: t.colors, state: t.state, logo: t.logo, crestText: t.crestText, crestSeed: t.crestSeed }))
  };
}
