// pass5_band_ab.mjs — PASS 5 band gate: AI-vs-AI league stats with the whole
// offense pass LIVE vs KILLED. Band rule (roadmap): the moment AI gains a new
// scheme layer — here the RPO conflict-read engine (every staff carries an
// identity-scaled rpoRate), the gadget tier (aggression-scaled gadgetRate),
// and choice routes on the quick-game concepts every staff already calls —
// prove the stat bands didn't move. Third AMPLIFIED arm (dials cranked) is
// the saturation guard from trait_band_ab precedent: even a gadget-happy
// league must stay inside 2x the envelope.
// Kill arm: __noRPOConflict (legacy RPO branch) + __noGadgets +
// __noChoiceRoutes. pass4_band_ab thresholds.
// Run: node tools/pass5_band_ab.mjs [N-games-per-arm]   (default 300)
...
function arm(mode) {
  // mode: 'live' | 'kill' | 'amp'
  globalThis.__noRPOConflict = mode === 'kill';
  globalThis.__noGadgets = mode === 'kill';
  globalThis.__noChoiceRoutes = mode === 'kill';
  ...
  try {
    for (let i = 0; i < N; i++) {
      ...
      if (mode === 'amp') {
        for (const s of [sH, sA]) { s.gameplan.rpoRate = 60; s.gameplan.gadgetRate = 12; }
      }
  ...
  } finally { delete globalThis.__noRPOConflict; delete globalThis.__noGadgets; delete globalThis.__noChoiceRoutes; }
  return acc;
}
...
const live = arm('live');
line('Pass 5 LIVE', live);
const dead = arm('kill');
line('Pass 5 KILLED', dead);
const amp = arm('amp');
line('AMPLIFIED (rpo60/gad12)', amp);
