...
try{
  ...
  await wizard.locator('#btn-mm-newcoach').click(); await wizard.locator('#mm-nc-first').fill('Logo'); await wizard.locator('#mm-nc-last').fill('Test'); await wizard.locator('#mm-nc-create').click();
  const worldButton=wizard.locator('[data-mm-world-new="1"]'); check(await worldButton.count()===1,'new dynasty entry is available'); await worldButton.click(); await wizard.locator('#ob-next-0').click(); await wizard.locator('[data-ob-challenge="takejob"]').click(); await wizard.locator('#ob-next-1').click(); await wizard.locator('[data-ob-state="CA"]').click(); await wizard.locator('[data-ob-div="D3"]').click(); await wizard.waitForTimeout(150);
