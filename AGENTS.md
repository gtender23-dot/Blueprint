# Blueprint: College Football Dynasty — project notes for Codex
...
```
node tools/_gate.mjs            # core tier — the per-change-set gate (~30-40 min)
node tools/_gate.mjs full       # deploy-level sweep (~2 h; full ⊇ core)
...
machine; `localOnly` entries (h2_shadow, recruit_calendar) are CPU-bound giants
skipped in the cloud — run them locally before a deploy. N overrides live in
